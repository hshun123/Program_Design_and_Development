...
Bus::Bus(std::string name, Route * out, Route * in,
        int capacity, double speed,  int strategy) {
  name_ = name;
  outgoing_route_ = out;
  incoming_route_ = in;
...
bool Bus::LoadPassenger(Passenger * new_passenger) {
  ...
  if (loader_->LoadPassenger(new_passenger, passenger_max_capacity_,
      &passengers_)) {  // Eric changed iteration step 1 # 2
  ...
  if (speed_ > 0) {
    ...
    if (distance_remaining_ <= 0) {
        ...
        if (outgoing_route_->IsAtEnd()) {
            ...
            if (!incoming_route_->IsAtEnd()) {
                ...
                if (passengers_handled != 0) {
                  ...
                  // Skipping the bus when no passenger is load or unload
                  std::cout << "skipping the stop " << ", id: "
                  << next_stop_->GetId() << ", bus name: "
                  << name_  << std::endl;
                }
        ...
        if (passengers_handled != 0) {
            distance_remaining_ = 0;
            did_move = true;
        } else {  // Skipping the bus when no passenger is load or unload
          std::cout << "skipping the stop " << ", id: " << next_stop_->GetId()
          << ", bus name: " << name_  << std::endl;
        }
...
void Bus::Update() {  // using common Update format
  Move();
  UpdateBusData();
  // possible place to put NotifyObservers
