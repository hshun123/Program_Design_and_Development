...
#include <sstream>
bool paused = false;  // global count, used for check pause/resume
...
// calls Pause() on the visualizationsumulator mySim
void PauseCommand::execute(MyWebServerSession* session,
  picojson::value& command, MyWebServerSessionState* state) {
    ...
    paused = !paused;  // value toggles between pause and resume
    mySim->Pause();
}
...
// Iteration 2 provided code from lab 14
// This class is a concrete observer of the IObserver.
class BusWebObserver : public IObserver {
 public:
    ...
    // This function notifies data it receives
